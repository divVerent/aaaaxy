...
// JumpPadVert, when hit by the player, sends the player up only.
// Note that sadly, JumpPadVerts are rarely ever useful in rooms that can be used in multiple orientations.
// May want to introduce required orientation like with checkpoints.
// Or could require player to hit jumppad from above.
type JumpPadVert struct {
...
func (j *JumpPadVert) Spawn(w *engine.World, sp *level.SpawnableProps, e *engine.Entity) error {
...
func (j *JumpPadVert) Despawn() {}

func (j *JumpPadVert) Update() {
...
func calculateJumpUp(delta m.Delta, heightParam int) m.Delta {
	...
	return m.Delta{DX: delta.DX, DY: vDY}
}

func (j *JumpPadVert) Touch(other *engine.Entity) {
	...
	delta := m.Delta{DX: p.ReadVelocity().DX, DY: j.Height}
	...
	if p.ReadOnGroundVec().DY < 0 {
		...
		p.SetVelocityForJump(m.FlipY().Apply(calculateJumpUp(m.FlipY().Apply(delta), j.Height)))
	} else {
		p.SetVelocityForJump(calculateJumpUp(delta, j.Height))
	}
	j.JumpSound.Play()
}

func init() {
	engine.RegisterEntityType(&JumpPadVert{})
}
