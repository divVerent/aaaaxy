...
func renderTextScreen(dst *ebiten.Image, titleFont, normalFont font.Face, text []string, pos m.Pos, centerX bool, lineHeight int, titleFG, titleBG, normalFG, normalBG color.Color) {
	x := pos.X
	nextIsTitle := true
	for i, line := range text {
		...
		if isTitle {
			titleFont.Draw(dst, line, m.Pos{X: x, Y: y}, centerX, titleFG, titleBG)
		} else {
			normalFont.Draw(dst, line, m.Pos{X: x, Y: y}, centerX, normalFG, normalBG)
		}
	}
}
