...
if [ -d .git ] && [ x"$AAAAXY_BUILD_USE_VERSION_FILE" != x'true' ]; then
	...
	while :; do
		...
		if ! git diff --quiet "$parent" HEAD -- . ':!docs' ':!io.github.divverent.aaaaxy.metainfo.xml' ':!third_party/SDL_GameControllerDB/assets/input' ':!.gitmoduleversions' ':!.lastreleaseversion' ':!.lastreleaseversioncode' ':!fastlane/metadata/android/en-US/changelogs'; then
